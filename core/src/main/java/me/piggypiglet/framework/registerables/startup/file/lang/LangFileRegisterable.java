...
public final class LangFileRegisterable extends StartupRegisterable {
    ...
    private void useDefaultLang(boolean external) {
        try {
            doConfig(
                    lang.getValues().stream().collect(Collectors.toMap(LangEnum::getPath, LangEnum::getPath)),
                    fileManager.loadConfig("lang", "/lang.json", external ? "lang.json" : null)
            );
        } catch (Exception e) {
            throw new RuntimeException("Something went dreadfully wrong when loading the default lang file. How disappointing :(\n" + e);
        }
    }
}
